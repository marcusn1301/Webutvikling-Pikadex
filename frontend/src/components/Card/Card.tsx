...
const Card = ({
  ...
  const [favoritesClicked, setFavoritesClicked] =
  ...
  const handleTypeColor = () => {
    return hexRgb(
      pokemonTypesColors[type_1 as keyof typeof pokemonTypesColors],
      {
        format: "css",
      }
    )
  ...
  return (
    <>
      {!isExpanded ? (
        ...
        <CardExpandContainer
          className="expandendCardContainer"
          backgroundColor={handleTypeColor()}
        >
      ...
      {isExpanded ? (
        <CardOverlay onClick={() => handleRemovePopup(!isExpanded)} />
      ) : (
        <></>
      )}
    </>
